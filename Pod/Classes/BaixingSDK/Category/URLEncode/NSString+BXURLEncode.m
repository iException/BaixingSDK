//
//  NSString+BXURLEncode.m
//  BaixingSDK
//
//  Created by phoebus on 9/28/14.
//  Copyright (c) 2014 baixing. All rights reserved.
//

#import "NSString+BXURLEncode.h"

static NSString* const kAFCharactersToBeEscapedInQueryString = @":/?&=;+!@#$()',*";
static NSString* const kAFCharactersToLeaveUnescapedInQueryStringPairKey = @"[].";

@implementation NSString (BXURLEncode)

//- (NSString *)urlEncode
//{
//    NSString *s = (__bridge_transfer NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,
//                                                                                        (CFStringRef)self,
//                                                                                        NULL,
//                                                                                        (CFStringRef)@"!*'();:@&=+$,/?%#[]",
//                                                                                        kCFStringEncodingUTF8);
//    return s;
//}

- (NSString*)urlEncode
{
    NSMutableString* output = [NSMutableString string];
    const unsigned char* source = (const unsigned char*)[self UTF8String];
    int sourceLen = (int)strlen((const char*)source);
    for (int i = 0; i < sourceLen; ++i) {
        const unsigned char thisChar = source[i];
        if (thisChar == ' ') {
            [output appendString:@"+"];
        }
        else if (thisChar == '.' || thisChar == '-' || thisChar == '_' || thisChar == '~' || (thisChar >= 'a' && thisChar <= 'z') || (thisChar >= 'A' && thisChar <= 'Z') || (thisChar >= '0' && thisChar <= '9')) {
            [output appendFormat:@"%c", thisChar];
        }
        else {
            [output appendFormat:@"%%%02X", thisChar];
        }
    }
    return output;
}

- (NSString*)urlDecode
{
    NSString* replaceWhiteSpace = [self stringByReplacingOccurrencesOfString:@"+" withString:@"%20"];
    return [replaceWhiteSpace stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
}

@end
